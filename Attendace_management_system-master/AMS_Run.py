...
window = tk.Tk()
window.title("FACE ATTENDENCE SYSTEM")

 #window.geometry('1280x720')
# window.configure(background='grey')
window_background_image = Image.open("OIP.jpg")
window_background_photo = ImageTk.PhotoImage(window_background_image)
window.background = window_background_photo
...
def manually_fill():
    ...
    def err_screen_for_subject():
        ...
        Button(ec, text='OK', command=ec_delete, fg="black", bg="green", width=9, height=1, activebackground="white",
    ...
    def fill_attendance():
        ...
        if subb=='':
            ...
            def create_csv():
                ...
                csv_name='D:\cprogram.c\Attendace_management_system-master\Attendance\Manually Attendance/'+DB_table_name+'.csv'
            ...
            def attf():
                import subprocess
                subprocess.Popen(r'explorer /select,"D:\cprogram.c\Attendace_management_system-master\Attendance\Manually Attendance\-------Check atttendance-------"')
    ...
    SUB_ENTRY = tk.Entry(sb, width=20, bg="black", fg="red", font=('times', 23, ' bold '))
    SUB_ENTRY.place(x=250, y=105)

    fill_manual_attendance = tk.Button(sb, text="Fill Attendance",command=fill_attendance, fg="white", bg="black", width=20, height=2,
                       activebackground="Red", font=('times', 15, ' bold '))
...
def subjectchoose():
    def Fillattendances():
        ...
        if time.time() < future:
            if sub == '':
                ...
                root.configure(background='snow')
                cs = 'D:\cprogram.c\Attendace_management_system-master/' + fileName
    ...
    def Attf():
        import subprocess
        subprocess.Popen(r'explorer /select,"D:\cprogram.c\Attendace_management_system-master\Attendance\-------Check atttendance-------"')
...
def admin_panel():
    ...
    def log_in():
        ...
        if username == 'kunal' :
            if password == 'kunal9525':
                ...
                cs = 'D:\cprogram.c\Attendace_management_system-master\StudentDetails/StudentDetails.csv'
                with open(cs, newline="") as file:
                    reader = csv.reader(file)
                    r = 0
    ...
    un_entr = tk.Entry(win, width=20, bg="sky blue", fg="red", font=('times', 23, ' bold '))
    un_entr.place(x=290, y=55)

    def c11():
        pw_entr.delete(first=0, last=22)

    pw_entr = tk.Entry(win, width=20,show="*", bg="sky blue", fg="red", font=('times', 23, ' bold '))
    ...
    Login = tk.Button(win, text="LogIn", fg="black", bg="lime green", width=20,
                       height=2,
                       activebackground="blue",command=log_in, font=('times', 15, ' bold '))
...
canvas = tk.Canvas(window, width=136, height=768)
canvas.pack(fill="both", expand=True)
canvas.create_image(6, 3, image=window.background, anchor="nw")
...
message = tk.Label(window, text="FACE ATTENDENCE SYSTEM", bg="cyan", fg="black", width=50,
                   height=1, font=('times', 10, 'italic bold '))

message.place(x=400, y=10)

Notification = tk.Label(window, text="All things good", bg="Green", fg="white", width=15,
                      height=3, font=('times', 17, 'bold'))

lbl = tk.Label(window, text="Enter Enrollment", width=15, height=1, fg="yellow", bg="blue", font=('times', 15, ' bold '))
...
txt = tk.Entry(window, validate="key", width=15, bg="gray", fg="yellow", font=('times', 15, ' bold '))
txt['validatecommand'] = (txt.register(testVal),'%P','%d')
txt.place(x=250, y=200)

lbl2 = tk.Label(window, text="Enter Name", width=15, fg="yellow", bg="blue", height=1, font=('times', 15, ' bold '))
lbl2.place(x=30, y=250)

txt2 = tk.Entry(window, width=15, bg="gray", fg="yellow", font=('times', 15, ' bold '))
...
# clearButton1 = tk.Button(window, text="Clear",command=clear1,fg="black"  ,bg="white"  ,width=10 ,height=1, activebackground = "Red" ,font=('times', 15, ' bold '))
